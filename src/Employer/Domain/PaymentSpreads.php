...
class PaymentSpreads implements JsonSerializable
{
    ...
    {
        Assertion::true(
            $this->min->isSameCurrency($this->max),
            sprintf(
                'Values must be of the same currency. Given "%s" and "%s"',
                $this->min->getCurrency(),
                $this->max->getCurrency()
            )
        );
        ...
        Assertion::greaterThan(
            ...
            $this->min->getAmount(),
            sprintf(
                'The maximum value must be greater than the minimum value. Given; min: (%s) max: (%s)',
                $this->min->getAmount(),
                $this->max->getAmount()
            )
        );
